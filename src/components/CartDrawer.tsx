...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  const shippingCost = items.length === 0 || isFreeShipping ? 0 : 7000;
  ...
  const handleCheckoutWhatsApp = () => {
    const lines = items
      .map(
        item =>
          `• ${item.fruit.name} (${item.fruit.presentation}) x${item.quantity}: $${(
            item.fruit.standardPrice * item.quantity
          ).toLocaleString('es-CO')} COP`
      )
      .join('%0A');

    const message = `*PEDIDO DIRECTO - FRESH PICK FRUTAS*%0A%0A*Productos:*%0A${lines}%0A%0A*Subtotal:* $${subtotal.toLocaleString(
      'es-CO'
    )} COP%0A*Envío:* ${
      isFreeShipping ? 'GRATIS' : '$7.000 COP'
    }%0A*TOTAL:* $${grandTotal.toLocaleString(
      'es-CO'
    )} COP%0A%0AHola! Deseo confirmar este pedido de frutas frescas para despacho a domicilio.`;

    window.open(`https://wa.me/573216920138?text=${message}`, '_blank');
  };
      ...
      <div className="w-full max-w-md bg-white h-full shadow-2xl flex flex-col justify-between animate-in slide-in-from-right duration-300">
        ...
        <div className="p-5 border-b border-stone-200 flex items-center justify-between bg-stone-50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold text-stone-900 font-display">Tu Carrito de Cosecha</h3>
          ...
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-4">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-blue-700 text-white text-xs font-bold hover:bg-blue-800 transition-colors cursor-pointer"
              >
            ...
            items.map(item => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-xs font-bold text-stone-900 truncate">{item.fruit.name}</h4>
          ...
          <div className="p-5 border-t border-stone-200 bg-stone-50 space-y-3">
            <div className="space-y-1 text-xs text-stone-600">
              ...
              <div className="flex justify-between items-center">
                <span>Envío:</span>
                {isFreeShipping ? (
                  <span className="text-blue-700 font-bold bg-blue-100 px-2 py-0.2 rounded">
                    ¡Gratis!
                  </span>
                ) : (
                  <span className="font-semibold text-stone-800">
